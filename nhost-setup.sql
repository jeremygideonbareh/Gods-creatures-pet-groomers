-- Helper: get current user UUID from Hasura session
CREATE OR REPLACE FUNCTION public.current_user_uuid() RETURNS uuid
  LANGUAGE sql STABLE
  AS $$ SELECT nullif(current_setting('hasura.user', true), '')::json->>'x-hasura-user-id' $$;

-- Backward-compat wrapper (returns text, uses cast internally)
CREATE OR REPLACE FUNCTION public.current_user_id() RETURNS text
  LANGUAGE sql STABLE
  AS $$ SELECT public.current_user_uuid()::text $$;
...
CREATE OR REPLACE FUNCTION update_site_content_timestamp() RETURNS trigger AS $$
...
CREATE POLICY "pets_insert_own" ON pets
  FOR INSERT WITH CHECK (user_id = public.current_user_uuid());

CREATE POLICY "pets_select_own" ON pets
  FOR SELECT USING (user_id = public.current_user_uuid());

CREATE POLICY "pets_update_own" ON pets
  FOR UPDATE USING (user_id = public.current_user_uuid());
...
CREATE TABLE IF NOT EXISTS bookings (
  ...
  notes TEXT DEFAULT '',
  advance_paid INTEGER DEFAULT 500,
  transaction_id TEXT NOT NULL,
  status TEXT DEFAULT 'pending_verification',
  addons JSONB DEFAULT '[]',
  total_price INTEGER,
  created_at TIMESTAMPTZ DEFAULT now(),
  CONSTRAINT bookings_transaction_id_key UNIQUE (transaction_id),
  CONSTRAINT bookings_status_check CHECK (status IN ('pending_verification', 'confirmed', 'cancelled'))
);
